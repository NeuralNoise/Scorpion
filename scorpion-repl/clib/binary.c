...
bool Binary::show = true;
long Binary::bytestatus = 0;
long Binary::bytesize = 0;
long Binary::sourcecount = 0;

int percentage()
{
    double a = 1.0, b = 1.0, c;
    a = a*Binary::bytestatus;
    b = b*Binary::bytesize;
...
std::string Binary::encode_str( const std::string &binary ) {
        ...
        for( size_t len = binary.size(); len--; ) {
             bytestatus++;
            if(show)
              printf("\rsar:  unpacking %lu file(s), %lu total bytes of data.  %d%s (%lu/%lu)", Binary::sourcecount, Binary::bytesize, percentage(), "%", Binary::bytestatus++, Binary::bytesize);
              //usleep(5);
            cout << std::flush;
