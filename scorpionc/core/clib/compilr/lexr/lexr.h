...
#ifndef INCLUDE_LEXR_H
#define INCLUDE_LEXR_H
...
namespace lexr
{
   ...
   public:
      ...
      unsigned long line_t, token_sz1;
   ...
   private:
      ...
      inline void scan_operator()
      {
         ...
         if ('<' == *s_itr_)
            ...
            t.set_operator(token_t::e_semicolon,s_itr_,s_itr_ + 1,base_itr_);
         else if ('&' == *s_itr_)
   ...
   namespace helper
   {
      ...
      inline void dump(lexr::generator& generator)
      {
         for (std::size_t i = 0; i < generator.size(); ++i)
         {
            lexr::token t = generator[i];
      ...
      public:
         ...
         inline int insert(const lexr::token& t0, const lexr::token& t1, lexr::token& new_token)
         {
            new_token.type     = lexr::token::e_mul;
            new_token.value    = "*";
            new_token.position = t1.position;
            bool match         = false;

            if (t0.type == lexr::token::e_symbol)
            {
            ...
            if (t1.type == lexr::token::e_symbol)
            {
               if (ignore_set_.end() != ignore_set_.find(t1.value))
               {
                 ...
                 if ((t0.type == lexr::token::e_number     ) && (t1.type == lexr::token::e_symbol     )) match = true;
            else if ((t0.type == lexr::token::e_number     ) && (t1.type == lexr::token::e_lbracket   )) match = true;
            else if ((t0.type == lexr::token::e_number     ) && (t1.type == lexr::token::e_lcrlbracket)) match = true;
            else if ((t0.type == lexr::token::e_number     ) && (t1.type == lexr::token::e_lsqrbracket)) match = true;
            else if ((t0.type == lexr::token::e_symbol     ) && (t1.type == lexr::token::e_number     )) match = true;
            else if ((t0.type == lexr::token::e_rbracket   ) && (t1.type == lexr::token::e_number     )) match = true;
            else if ((t0.type == lexr::token::e_rcrlbracket) && (t1.type == lexr::token::e_number     )) match = true;
            else if ((t0.type == lexr::token::e_rsqrbracket) && (t1.type == lexr::token::e_number     )) match = true;
            else if ((t0.type == lexr::token::e_rbracket   ) && (t1.type == lexr::token::e_symbol     )) match = true;
            else if ((t0.type == lexr::token::e_rcrlbracket) && (t1.type == lexr::token::e_symbol     )) match = true;
            else if ((t0.type == lexr::token::e_rsqrbracket) && (t1.type == lexr::token::e_symbol     )) match = true;
      ...
      public:
         ...
         inline bool join(const lexr::token& t0, const lexr::token& t1, lexr::token& t)
         {
            //': =' --> ':='
            if ((t0.type == lexr::token::e_colon) && (t1.type == lexr::token::e_eq))
            {
               t.type = lexr::token::e_assign;
               t.value = ":=";
               t.position = t0.position;
               return true;
            }
            //'> =' --> '>='
            else if ((t0.type == lexr::token::e_gt) && (t1.type == lexr::token::e_eq))
            {
               t.type = lexr::token::e_gte;
               t.value = ">=";
               t.position = t0.position;
               return true;
            }
            //'< =' --> '<='
            else if ((t0.type == lexr::token::e_lt) && (t1.type == lexr::token::e_eq))
            {
               t.type = lexr::token::e_lte;
               t.value = "<=";
               t.position = t0.position;
               return true;
            }
            //'= =' --> '=='
            else if ((t0.type == lexr::token::e_eq) && (t1.type == lexr::token::e_eq))
            {
               t.type = lexr::token::e_eq;
               t.value = "==";
               t.position = t0.position;
               return true;
            }
            //'! =' --> '!='
            else if ((static_cast<char>(t0.type) == '!') && (t1.type == lexr::token::e_eq))
            {
               t.type = lexr::token::e_ne;
               t.value = "!=";
               t.position = t0.position;
               return true;
            }
            //'< >' --> '<>'
            else if ((t0.type == lexr::token::e_lt) && (t1.type == lexr::token::e_gt))
            {
               t.type = lexr::token::e_ne;
               t.value = "<>";
               t.position = t0.position;
      ...
      public:
         ...
         bool operator()(const lexr::token& t)
         {
            if (!t.value.empty()                         &&
                (lexr::token::e_string != t.type)     &&
                (lexr::token::e_symbol != t.type)     &&
                details::is_bracket(t.value[0])
               )
            {
               char c = t.value[0];

                    if (t.type == lexr::token::e_lbracket)    stack_.push(')');
               else if (t.type == lexr::token::e_lcrlbracket) stack_.push('}');
               else if (t.type == lexr::token::e_lsqrbracket) stack_.push(']');
      ...
      public:
         ...
         bool add_replace(const std::string& target_symbol,
                          ...
                          const lexr::token::token_type token_type = lexr::token::e_symbol)
         {
            replace_map_t::iterator itr = replace_map_.find(target_symbol);
      ...
      private:
         ...
         bool modify(lexr::token& t)
         {
            if (lexr::token::e_symbol == t.type)
            {
      ...
      private:
         ...
         typedef std::pair<lexr::token::token_type,lexr::token::token_type> token_pair_t;
         typedef std::set<token_pair_t> set_t;

      public:

         sequence_validator()
         : lexr::token_scanner(2)
         {
            add_invalid(lexr::token::e_number,lexr::token::e_number);
            add_invalid(lexr::token::e_string,lexr::token::e_string);
            add_invalid(lexr::token::e_number,lexr::token::e_string);
            add_invalid(lexr::token::e_string,lexr::token::e_number);
            add_invalid(lexr::token::e_string,lexr::token::e_colon);
            add_invalid(lexr::token::e_colon,lexr::token::e_string);
            add_invalid_set1(lexr::token::e_assign);
            add_invalid_set1(lexr::token::e_shr   );
            add_invalid_set1(lexr::token::e_shl   );
            add_invalid_set1(lexr::token::e_lte   );
            add_invalid_set1(lexr::token::e_ne    );
            add_invalid_set1(lexr::token::e_gte   );
            add_invalid_set1(lexr::token::e_lt    );
            add_invalid_set1(lexr::token::e_gt    );
            add_invalid_set1(lexr::token::e_eq    );
            add_invalid_set1(lexr::token::e_comma );
            add_invalid_set1(lexr::token::e_add   );
            add_invalid_set1(lexr::token::e_sub   );
            add_invalid_set1(lexr::token::e_div   );
            add_invalid_set1(lexr::token::e_mul   );
            add_invalid_set1(lexr::token::e_mod   );
            add_invalid_set1(lexr::token::e_pow   );
            add_invalid_set1(lexr::token::e_colon );
         }
         ...
         bool operator()(const lexr::token& t0, const lexr::token& t1)
         {
         ...
         std::pair<lexr::token,lexr::token> error(const std::size_t index)
         {
            ...
            {
               static const lexr::token error_token;
      ...
      private:
         ...
         void add_invalid(lexr::token::token_type base, lexr::token::token_type t)
         {
            invalid_comb_.insert(std::make_pair(base,t));
         }

         void add_invalid_set1(lexr::token::token_type t)
         {
            add_invalid(t,lexr::token::e_assign);
            add_invalid(t,lexr::token::e_shr   );
            add_invalid(t,lexr::token::e_shl   );
            add_invalid(t,lexr::token::e_lte   );
            add_invalid(t,lexr::token::e_ne    );
            add_invalid(t,lexr::token::e_gte   );
            add_invalid(t,lexr::token::e_lt    );
            add_invalid(t,lexr::token::e_gt    );
            add_invalid(t,lexr::token::e_eq    );
            add_invalid(t,lexr::token::e_comma );
            add_invalid(t,lexr::token::e_div   );
            add_invalid(t,lexr::token::e_mul   );
            add_invalid(t,lexr::token::e_mod   );
            add_invalid(t,lexr::token::e_pow   );
            add_invalid(t,lexr::token::e_colon );
         }

         bool invalid_bracket_check(lexr::token::token_type base, lexr::token::token_type t)
         {
            if (details::is_right_bracket(static_cast<char>(base)))
            {
               switch (t)
               {
                  case lexr::token::e_string : return true;
                  case lexr::token::e_assign : return true;
            ...
            else if (details::is_left_bracket(static_cast<char>(base)))
            {
               ...
               else
               {
                  switch (t)
                  {
                     case lexr::token::e_number : return false;
                     case lexr::token::e_symbol : return false;
                     case lexr::token::e_string : return false;
                     case lexr::token::e_add    : return false;
                     case lexr::token::e_sub    : return false;
                     case lexr::token::e_colon  : return false;
                     default                       : return true;
                  }
               }
            }
            else if (details::is_right_bracket(static_cast<char>(t)))
            {
               switch (base)
               {
                  ...
                  case lexr::token::e_symbol : return false;
                  case lexr::token::e_string : return false;
                  case lexr::token::e_eof    : return false;
                  case lexr::token::e_colon  : return false;
                  default                       : return true;
               }
            }
            else if (details::is_left_bracket(static_cast<char>(t)))
            {
               switch (base)
               {
                  case lexr::token::e_rbracket    : return true;
                  case lexr::token::e_rsqrbracket : return true;
                  case lexr::token::e_rcrlbracket : return true;
                  default                            : return false;
               }
            }
         ...
         set_t invalid_comb_;
         std::deque<std::pair<lexr::token,lexr::token> > error_list_;

      };

      struct helper_assembly
      {
         inline bool register_scanner(lexr::token_scanner* scanner)
         {
         ...
         inline bool register_modifier(lexr::token_modifier* modifier)
         {
         ...
         inline bool register_joiner(lexr::token_joiner* joiner)
         {
         ...
         inline bool register_inserter(lexr::token_inserter* inserter)
         {
         ...
         inline bool run_modifiers(lexr::generator& g)
         {
            error_token_modifier = reinterpret_cast<lexr::token_modifier*>(0);
            bool result = true;

            for (std::size_t i = 0; i < token_modifier_list.size(); ++i)
            {
               lexr::token_modifier& modifier = (*token_modifier_list[i]);
         ...
         inline bool run_joiners(lexr::generator& g)
         {
            error_token_joiner = reinterpret_cast<lexr::token_joiner*>(0);
            bool result = true;

            for (std::size_t i = 0; i < token_joiner_list.size(); ++i)
            {
               lexr::token_joiner& joiner = (*token_joiner_list[i]);
         ...
         inline bool run_inserters(lexr::generator& g)
         {
            error_token_inserter = reinterpret_cast<lexr::token_inserter*>(0);
            bool result = true;

            for (std::size_t i = 0; i < token_inserter_list.size(); ++i)
            {
               lexr::token_inserter& inserter = (*token_inserter_list[i]);
               inserter.reset();
               inserter.process(g);
               if (!inserter.result())
               {
         ...
         inline bool run_scanners(lexr::generator& g)
         {
            error_token_scanner = reinterpret_cast<lexr::token_scanner*>(0);
            bool result = true;

            for (std::size_t i = 0; i < token_scanner_list.size(); ++i)
            {
               lexr::token_scanner& scanner = (*token_scanner_list[i]);
         ...
         std::deque<lexr::token_scanner*>  token_scanner_list;
         std::deque<lexr::token_modifier*> token_modifier_list;
         std::deque<lexr::token_joiner*>   token_joiner_list;
         std::deque<lexr::token_inserter*> token_inserter_list;

         lexr::token_scanner*  error_token_scanner;
         lexr::token_modifier* error_token_modifier;
         lexr::token_joiner*   error_token_joiner;
         lexr::token_inserter* error_token_inserter;
      };
   }
