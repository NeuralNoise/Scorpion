...
int svm_main = 0, d_max_precision=16;
u4_d op_ags;
...
int Scorpion_InvokeMethod(long ptrValue, ScorpionVmState* vm, long line){
    ...
    if(includep){
        ...
        stringstream s;
        s << gSvm.mtds[ptrValue].package << "." << gSvm.mtds[ptrValue].name;
    ...
    gSvm.mtds[ptrValue].ret = vm->k;
    vm->k = gSvm.mtds[ptrValue].jmp;
...
void return_method(long ptrValue, ScorpionVmState* vm){
    ...
    vm->k = gSvm.mtds[ptrValue].ret;
}

ScorpionVM::memory::ObjectContainer& MOJ(long i) { 
    if(i>gSvm.env->bitmap.base)  
    {
       stringstream ss;
       ss << "pointer to address: " << i << " is out of allocated data range.";
       Exception(ss.str(), "MethodNotFoundException");
    }
    return gSvm.env[0].m_heap[i]; 
}

bool isnumber(ScorpionVM::memory::ObjectContainer o)
{ return o.valuetype->type != ScorpionVM::memory::schema::ObjectSchema::SSTRING; }

void checkobj(ScorpionVM::memory::ObjectContainer o)
{ 
    if(!o.init) 
    { // TODO: Figure out why it is not initalized
       Exception("Cannot operate on null value.", "IllegalTypeException");
    }
...
{
   if(!MOJ(addr).init) Exception("Cannot operate on null object.", "NullPointerException");
   if(MOJ(addr).array)
   {
  ...
  if(form == 'i')
    cout << (sdouble) ((sint) MOJ(addr).schema_value()));
  else if(form == 's')
    cout << (sdouble) ((sshort)  MOJ(addr).schema_value());
  else if(form == 'B')
    cout << (sdouble) ((sbyte)  MOJ(addr).schema_value());
  else if(form == 'l')
    cout << (sdouble) ((slong)  MOJ(addr).schema_value());
  else if(form == 'p')
    printf("%#x", (unsigned int) addr);
  else if(form == 'S' || MOJ(addr).valuetype->type == TYPEDEF_STRING)
    ...
    cout << std::setprecision((d_max_precision/2) - 1) << (sfloat) svmGetGenericValue(MOJ(addr));
  else if(form == 'd' || MOJ(addr).instanceData.byte1 == TYPEDEF_GENERIC_DOUBLE)
    cout << std::setprecision(d_max_precision) << (double) svmGetGenericValue(MOJ(addr));
  else
   ...
   // if(gSvm.Debug)
    ...
    vmdispatch(vmstate->i){ 
       ...
       dispatchop(OP_STRCONST,
           ...
           op_ags.byte2=vmstate->bytestream.valueAt(vmstate->k++);
           u1 sz;
           string data = getStr(vmstate,op_ags.byte2);
           sz.byte1 = data.size();
           SVM_OBJECT_INIT(MOJ(op_ags.byte1), TYPEDEF_STRING, sz);
           assign(MOJ(op_ags.byte1), data);
       )
       ...
       dispatchop(OP_CAST,
           ...
           int cast = op_ags.byte3;
           if(cast == 1) newData = (schar) data;
           else if(cast == 2) newData = (sbyte) data;
           else if(cast == 3) newData = (sint) data;
           else if(cast == 4) newData = (sshort) data;
           else if(cast == 5) newData = (slong) data;
           else if(cast == 6) newData = (slong) op_ags.byte2;
           else if(cast == 7) newData = (sbool) data;
           else if(cast == 8) newData = (sfloat) data;
           else newData = data;
           
           svmSetGenericValue(MOJ(op_ags.byte1), newData);
       )
       dispatchopnb(OP_JIT,
           op_ags.byte1=vmstate->bytestream.valueAt(vmstate->k++);
           op_ags.byte2=vmstate->bytestream.valueAt(vmstate->k++);
