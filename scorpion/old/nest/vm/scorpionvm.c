...
void Init_CreateScorpionVM(ScorpionVmState *vm, ScorpionVM::memory::environment::scorpion_env* env, 
    XSO* f, const char** args, int ags_t)
{
    /*
    ...
    */
    ...
    ScorpionVM::memory::BlockAllocator
        <ScorpionVM::memory::scorpion_env> env_allocator;
    ScorpionVM::memory::BlockAllocator<ScorpionVmState> vm_allocator;
    ScorpionVM::memory::BlockAllocator<XSO> image_allocator;
        
    env = env_allocator.calloc(SINGLE_ENVIRONMENT, 0);
    gSvm.vmCount++;
    gSvm.envCount++;
    ...
    if(XSO_PATH){
       ...
       f = image_allocator.calloc(1, 0); // we only need 1 XSO processor
     ...
     status = gSvm.env->InitEnvironmentSetup("ScorpionVirtualMachine"); 
     
     alog.setClass("ScorpionEnv");
     alog.ALOGV("Setting up new environment.");
    
    
      if(env->alloc(options.minHeap, options.maxHeap, 0xfff) != 0){
         alog.setClass("ScorpionEnv");
...
int Init_StartScorpionVM()
{
    ...
    */
    ...
    // TODO: push arguments on the stack to be processed by main method
      ...
      Exception::trace.addproto(main.str(), gSvm.mtds[0].clazz, 0, false); 
    ...
    //        result = 1;
     //   }
      ...
      if(gSvm.env != nullptr){ // shut down all block table structures 
         if(sp >= 0){
             unsigned long address = gSvm.env->getBitmap().stack->plong[sp--]; // simple stack pop
