 ...
 */
 ...
 void list_handler(std::string fail)
 {
     cout << "failed " << fail << endl;
     g_Svm.UnspecifiedException._throw_(g_Svm.vmstates);
 }
...
bool scorpion_strcompare(long op, string str, string str2)
{
 ...
 bool scorpion_Vcompare(long id, scorpion_env* env, ScorpionVM* vm)
 {
     
     long op = vm->image_stream.valueAt(vm->pc++);
     vm->pc++;
     long id2 = vm->image_stream.valueAt(vm->pc++);
     long id3 = vm->image_stream.valueAt(vm->pc++);
 ...
 void xso_exec(scorpion_env* env, ScorpionVM* vm_state)
 {
    ...
    for( ;; )
    {
      newframe:
       ...
       if(vm_state->state== vm_state_normal) goto run;
      ...
      //  printf("Instruction %d\n", (unsigned int)i);
       switch( i )
       {
          ...
          case OP_INC: {
              env->m_heap[(long)vm_state->image_stream.valueAt(vm_state->pc++)].schema_inc();
              continue;
          }
          case OP_DEC: {
          ...
          case OP_CMP: {
              long id = vm_state->image_stream.valueAt(vm_state->pc+1);
              env->m_heap[id].set_schema_(scorpion_Vcompare(id,env,vm_state));
              continue;
          }
          default: {
              Protect(vm_state, i);
